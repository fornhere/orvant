...
def run_demo(root):
    ...
    with tempfile.TemporaryDirectory(prefix='proje-demo-input-') as td:
        ...
        def event(action, **fields):
            revision = json.loads((root / '.project/state.json').read_text())['revision']
            payload = {'action': action, 'actor':'demo-runner',
                       'reason':'Sentetik, kontrollü iş akışı deneyi', **fields}
            event_file.write_text(json.dumps(payload, ensure_ascii=False))
        ...
        inspected = json.loads((root / 'data/tools.json').read_text())
        ...
        assert 'sentetik' in (root / 'demo-notu.md').read_text()
        ...
        (root / 'KANIT-DEGISMEDEN-ONCE.md').write_text(clean_context)
        ...
        (root / 'GUNCEL-DURUM.md').write_text(call(['context', root]).stdout)
